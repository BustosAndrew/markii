...
**Connect delivery is proven as of 2026-09-08.** Every earlier live Connect row carried a synthetic id — probes POSTed straight at the URL — so nothing had shown that a genuine Stripe-generated, Stripe-signed, Stripe-delivered Connect event survives the receiver. `stripe trigger invoice.created --stripe-account acct_…` against `stripe listen --forward-connect-to` produced `evt_1UDIuh…`: verified, tenant resolved from the event's `account`, and routed to the right branch ("Connected-account invoice is not for a subscription" — correct, since the fixture builds a standalone invoice). That is test mode through the CLI's ephemeral endpoint, not the production endpoint, so what it proves is the **receiver**; the production endpoint's *subscription* is proven separately by `pnpm stripe:webhooks`. Between them the chain is covered end to end.

**The API version is now `2026-07-29.dahlia`, matching what the webhook endpoints send** (2026-09-08). Both live endpoints render events as Dahlia, and an endpoint's explicit version wins over the account default — so the handlers were parsing Dahlia bodies while every outbound call spoke Basil. Two versions in one integration invites a handler written against a shape the API never returns.

The jump crosses **two** breaking releases, Clover and Dahlia, and every breaking change in both was checked against the surfaces this code touches: subscription-item `current_period_*`, `invoice.parent.subscription_details`, `create_preview`'s `subscription_details` parameters, and `POST /v1/invoiceitems` are all unchanged. Two came close and missed — Dahlia removes `payment_method_types` on PaymentIntents, which `stripe-charges.ts` does not send (it uses `automatic_payment_methods`), and Clover makes flexible billing mode the default, which is already the shape read here because periods come off the subscription **item**, never the subscription.

**Clover's flexible billing mode is now the default for anything newly created**, which is the one behavioural consequence of the move. Subscriptions made from here — Markii's own plans and shopper memberships alike — are flexible where a Basil-era one was classic; billing mode is fixed at creation, so existing subscriptions are untouched and the two kinds coexist permanently. It changes proration and period behaviour, not any field read here, since periods already come off the subscription item. Suspect it first if a proration preview ever disagrees with the final charge.

**Verified against Stripe rather than reasoned about alone**: `pnpm stripe:prices` reads every plan Price, and `tests/integration/stripe-fee-invoice.test.ts` (`MARKII_STRIPE_TESTS=1`) creates a real test-mode customer, subscription and invoice item and asserts the fee line against Stripe — all under the new pin. The pin lives in five files; `lib/billing/stripe-billing.ts` carries the reasoning and the rest point at it.
...
and `tax-shipping.ts`). **Digital delivery screens have since landed** — verified 2026-08-17:

...
export default async function FinancesDetailPage({
  ...
  if (error || !data) {
    return (
      <div>
        <PageHeader title="Site finances" description={slug} />
        <FetchError
  ...
  return (
    <div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <Stat
          label="Total"
          value={<MoneyText cents={data.balance.totalCents} />}
        />
        <Stat
          label="x402"
          value={
            <MoneyText cents={data.balance.x402Cents} currency="USDC" />
          }
        />
        <Stat
          label="Fiat"
          value={<MoneyText cents={data.balance.fiatCents} />}
        />
      </div>

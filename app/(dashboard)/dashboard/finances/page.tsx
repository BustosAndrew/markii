...
export default async function FinancesPage({
  ...
  return (
    <div>
      <PageHeader
        title="Finances"
        description="Balances across sites — x402 and fiat."
      />
      ...
      {error ? (
        <FetchError title="Finances unavailable" message={error} />
      ) : null}
      ...
      {!error && data ? (
        <>
          <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <Stat
              label="Total balance"
              value={<MoneyText cents={data.totalBalanceCents} />}
            />
            <Stat
              label="x402"
              value={
                <MoneyText cents={data.x402BalanceCents} currency="USDC" />
              }
            />
            <Stat
              label="Fiat"
              value={<MoneyText cents={data.fiatBalanceCents} />}
            />

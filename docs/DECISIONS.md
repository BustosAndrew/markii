...
| D45 | 2026-09-04 | **One free month per merchant account, no card, then the storefront stops** (owner). Every organization gets a calendar month from signup; when it lapses with nothing bought the stores stop serving and stop accepting orders, and subscribing reinstates everything immediately. **This ends an accidental free tier**: `organizations.plan_id` defaults to `starter` and `entitlementsFor` reads it directly, so until now an org that never subscribed kept full Starter entitlements — one storefront, a $1,000 threshold — forever, and `FLOOR_PLAN` quietly meant "free". **Standing is derived per request, never stored** (`lib/billing/standing.ts`): nothing in this codebase runs on a clock that could flip a stored flag at the moment a trial lapses, so a stored status would keep a store online long after it ran out — the same reasoning as membership status in D34. **The trial has no Stripe object behind it**, because signup takes no card and so there is nothing for Stripe to schedule; `free_trial_ends_at` is therefore a new column and deliberately **not** `trial_ends_at`, which mirrors Stripe and is overwritten by every `customer.subscription.*` webhook. **Enforcement is one check in `invokeAction`**, which §22 rule 1 makes the only mutation path, so it covers UI, HTTP API, agents and MCP at once; `billing.*` is exempt or the merchant would be locked out of the only door, and **reads are never gated** — holding a store is a commercial measure and must not look like holding their data hostage. Answers **402 `TRIAL_ENDED`**, not 403: permissions are fine and a second factor cannot help. **Warning is separate from enforcement** — the daily cron only mails, so if it never runs a merchant loses a warning rather than their store. **A halted store stops earning, not just selling** (owner, same day): membership **renewals** are refused and **digital downloads** are paused, alongside new checkouts. Both gate on `siteHalted`, which merges the billing hold with the merchant's own `paused` status — so a merchant who pauses their own store stops renewing too. **The charge is stopped before it happens** (owner, same day — this closed the cost first accepted here). `invoice.created` on the connected account is the pre-charge hook: Stripe drafts a subscription invoice about an hour before finalising it, and that is the only moment a payment can be prevented rather than regretted. A halted store's member subscriptions are put into `pause_collection[behavior]=void`, so **no invoice is ever charged**. `void` rather than `keep_as_draft` so a merchant returning after three months does not have Stripe hand their members three back-dated invoices at once. **Pause, never cancel:** cancelling is irreversible for the merchant — they would not get their subscribers back — so Markii must not destroy a book of recurring revenue over a lapsed trial. `syncMembershipCollection` reconciles in both directions from one idempotent pass, called when the merchant's own plan changes and when they pause or un-pause a store. The `invoice.paid` refusal stays as the **fallback** for anything that slips the window: it still declines to extend, records why in `stripe_webhook_events`, and does not meter — Markii will not recognise revenue against a threshold for access it refused. Downloads answer **409, not 404** — the grant, its remaining count and its expiry are untouched and the link works again when the store does. **Accepted cost:** a merchant who ignores both the banner and the email finds their storefront dark, and shoppers see "temporarily unavailable" with no reason given — the reason is Markii's business with the merchant, not something to publish to their customers | `docs/PRICING.md`, `lib/billing/standing.ts`, `drizzle/0035_free_trial.sql`, `CLAUDE.md` |
| D44 | 2026-08-16 | **Customer mail sends from the storefront's Markii address when the merchant has no verified domain** (owner) — amending G1, which previously refused it. Applies to *all* merchant mail, not just account mail: order confirmations, shipping and refund notices, and digital delivery. **The argument is not competitor parity** — `docs/COMPETITORS.md` records nothing about competitors' transactional email, so that claim was not available to lean on. It is that a store which takes an order and sends **nothing** is broken: the buyer has paid and heard silence, and for a digital product the missing email *is* the product, since the download link never arrives. Silence is a worse failure than an unbranded sender. **What G1 still forbids is unchanged**: never bare `markii.shop`, never Resend for merchant mail — the fallback is `{slug}.{ROOT_DOMAIN}`, per storefront, still SES. **The cost, accepted knowingly:** those bounces land on Markii's shared SES reputation, and SES suspends account-wide, so one merchant's bad list can still cost every merchant their receipts — which is why suppression (§24) and the `UNVERIFIED_SENDING_DOMAIN` finding both stay loud, and why D43 keeps *campaigns* off this path entirely. Verified domains always win when present | `docs/API.md` §24/§9, `lib/email/index.ts`, `CLAUDE.md` |
| D43 | 2026-08-15 | **Merchant email marketing campaigns are a delayed feature — do not build** (owner). Sharpens D27 from "not at launch" into an explicit deferral of the same kind as D33: nothing in broadcast sending, list management, segmentation, or campaign analytics is to be built, and **no schema is to anticipate it**. Transactional merchant mail (§24) is unaffected and stays on SES from the merchant's own verified domain; ESP integration as a Channel remains the answer for merchants who need campaigns today. Four prerequisites are recorded so the deferral is reversible without re-deriving them: a **consent/subscription model** (`customers` records no marketing opt-in, and CAN-SPAM/GDPR make sending without one a legal exposure, not a bug), **per-campaign unsubscribe with `List-Unsubscribe`** (a suppression list keyed to transactional bounces is not an opt-out register), a **scheduler** (nothing here runs jobs except the billing cron — D41), and **reputation isolation**, which is the sharp one: SES suspends on account-wide bounce and complaint rates, so one merchant blasting a stale list can take down **every** merchant's order confirmations. Campaign sending therefore cannot share the transactional path without dedicated IPs or per-merchant sub-accounts | `docs/API.md` §24, `docs/PLAN.md`, `CLAUDE.md` |
...
| D45 | 2026-08-17 | **Stripe Tax runs on the *merchant's* connected account, never Markii's** (§18.6, G3's merchant-side counterpart). The merchant is the seller of record under Connect Standard (G2), so their registrations decide what is owed, their account is billed per calculation, and the transactions backing their filings land in their reports — a calculation on the platform account would answer with **Markii's** tax position and present it to a shopper as their own. Three consequences are load-bearing. **(1) A calculation is a quote; a transaction is the filing record**, created only after payment succeeds — at quote time it reports tax on every abandoned basket, and skipped entirely it leaves a merchant charging tax correctly with nothing to file, unreconstructable because calculations expire in 90 days. Refunds reverse it. Both post-payment calls are **post-commit and cannot fail a transaction**: a settled checkout must not error over a reporting call, so the residual risk runs toward a findable gap (`orders.tax_transaction_id` / `refunds.tax_reversal_id` null) rather than a fabricated success. **(2) Calculations are cached per cart**, because Stripe bills the merchant per calculation and cart pricing runs on every render — and **per cart, never per store**, since a calculation converts into exactly one transaction and a shared id silently drops the second sale from the report. **(3) Recurring memberships are taxed by Stripe or not sold**: nothing here runs on a clock, so there is no moment months out at which Markii could tax a renewal, and a `manual`-rate store is refused with `409` rather than selling a membership taxed once and silently never again — a **behaviour change**; before this they were untaxed on every store. `activeRegistrations: 0` is surfaced separately from activation because Stripe Tax with no registration calculates a legitimate zero everywhere, so the store looks configured and collects nothing until filing time | `docs/API.md` §18.6, `lib/payments/stripe-tax.ts`, `lib/commerce/tax.ts`, `CLAUDE.md`, `docs/FRONTEND.md` |
| D46 | 2026-08-17 | **`variants.taxable: false` stays honoured on Stripe Tax only — the manual path keeps ignoring it** (owner, asked and answered). Not an oversight and not a deferral with a date: the manual engine is *structurally* incapable of per-line tax, because one rate over one base has nowhere to express an exemption. Honouring the flag there would fix the narrow case where exempt items are at exactly 0% while leaving the common one — two different non-zero rates, clothing against food — just as wrong, and would make the manual path **look** more capable than it is. A merchant who needs per-item tax treatment has by definition outgrown a single flat rate, and moving them to Stripe Tax is the answer the variant editor already gives them at the point they set the flag. **The disclosure is what makes this acceptable**, and it is provider-specific copy in `components/dashboard/variant-editor.tsx`, not a footnote — if that copy is ever removed, this decision has to be revisited, because an undisclosed mismatch between a merchant's setting and what a shopper is charged is a different thing entirely. Pinned by `tests/integration/pricing.test.ts` (\"taxes a non-taxable variant under manual rates, deliberately\"): the obvious \"fix\" cuts collected tax by 83% on a mixed basket, silently, on every live manual-rate store | `docs/API.md` §18.6, `lib/commerce/pricing.ts`, `components/dashboard/variant-editor.tsx`, `CLAUDE.md` |

...
| **Screens still stubbed** | Invoices, org switcher | Invoices are Stripe-gated (`configuration_required`, not "coming soon"). **The switcher is buildable now** — `POST /api/org/switch` is live and `/api/me` already returns every org the user belongs to |
